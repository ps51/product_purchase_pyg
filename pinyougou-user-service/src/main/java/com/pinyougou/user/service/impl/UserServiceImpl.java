...
public class UserServiceImpl implements UserService {
	...
	public void createSmsCode(final String phone) {
		//1.生成一个6位的随机数(验证码)   生成一个1000000以内的整数   Math：数学类
		//Math.random()生成一个0-1之间的小数
		final String smscode = (long)(Math.random()*1000000) + "";
		//2.将验证码放入redis
		redisTemplate.boundValueOps("SMSCODE:"+phone).set(smscode);
		redisTemplate.expire("SMSCODE:"+phone,120, TimeUnit.SECONDS);
		//3.将短信内容发送给activeMQ
	...
	public boolean checkSmsCode(String phone, String code) {
		String smscode = (String)redisTemplate.opsForValue().get("SMSCODE:"+phone);
		if(smscode == null){
			return false;
		}
